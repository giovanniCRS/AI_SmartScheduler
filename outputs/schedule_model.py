...
# Fairness adjustment: Increase weights for least-satisfied workers (1, 2, 5, 8, 9, 13, 15, 17, 18, 19)
...
least_satisfied_workers = [1, 2, 5, 8, 9, 13, 15, 17, 18, 19]

objective_terms = []
for w in range(num_workers):
    pref = preferences[w]
    
    # Determine weights based on fairness adjustment
    if w in least_satisfied_workers:
        night_weight = 20
        afternoon_weight = 8
        morning_weight = 8
        weekend_weight = 6
        holiday_base_weight = 10
    else:
        night_weight = 10
        afternoon_weight = 4
        morning_weight = 4
        weekend_weight = 3
        holiday_base_weight = 5
    ...
    holiday_cost = int(round(holiday_base_weight * (1 - pref.get("holiday_tolerance", 0.5))))
    for d in holidays:
